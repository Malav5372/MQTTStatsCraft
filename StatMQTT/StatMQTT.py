#!/usr/bin/env python
#
# 1. Read configuration pointed to by StatMQTTCONFIG env var, or use `StatMQTT.conf`
#    by default.
...
def run() -> None:
    ...
    topic_prefix = cf.get(
        'mqtt_topic_prefix', f'StatMQTT/{socket.gethostname()}/')
    ...
    #
    mqttc = MqttClient(
        cf.get('mqtt_clientid', 'StatMQTT-%s' % os.getpid()),

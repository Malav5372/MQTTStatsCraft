...
def load_config() -> Config:
    ...
    StatMQTT_conf_path = os.getenv(
        'StatMQTTCONFIG', os.path.join(dirname, 'StatMQTT.conf'))
    ...
    try:
        ...
        logging.debug("Loading app config '%s'", StatMQTT_conf_path)
        return Config(StatMQTT_conf_path)

    except Exception as e:
        print(f"Cannot load configuration from file {StatMQTT_conf_path}: {e}")
        sys.exit(2)
